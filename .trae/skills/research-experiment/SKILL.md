...
| `hardware_config.md` | 硬件检测和自动优化的配置报告 |
...
### Step 0: 硬件自动检测与配置优化
首先运行硬件检测，根据用户电脑配置自动调整实验策略：

```typescript
import { detectHardware, getOptimalExperimentConfig, generateHardwareReport } from 'scientify-trae';
...
// 保存硬件报告
await fs.writeFile('hardware_config.md', report);
console.log('✅ 硬件检测完成，已生成配置报告 hardware_config.md');
```

向用户展示硬件配置和推荐的实验参数，确认后再继续。所有后续步骤使用自动优化后的配置。
...
- epoch 数使用推荐值: `--epochs ${config.epochs.full}`
- 启用混合精度训练（如果支持）: `--mixed-precision ${config.mixedPrecision}`
- 梯度累积步数: `--gradient-accumulation ${config.gradientAccumulationSteps}`
...
**仅当配置中 enableAblation = true 时执行**，否则跳过此步骤。

如果硬件配置支持消融实验，根据 plan_res.md 中的消融计划，执行 2-3 个消融实验：

对每个消融：
1. 修改代码（注释/替换对应组件）
2. 执行 ${config.epochs.ablation} epoch 快速验证
3. 记录结果

```bash
# Example: 去掉 attention module
python3 run.py --epochs ${config.epochs.ablation} --ablation no_attention --batch-size ${config.batchSize} --mixed-precision ${config.mixedPrecision}
```

### Step 4: 实验分析→补充实验迭代（最多 2 轮）

**⚠️ 仅当配置中 enableSupplementaryExperiments = true 时执行**，否则跳过此步骤。

**这是 Novix Exp Analyzer 机制 — 对已有结果进行分析，提出补充实验，执行后再分析。**
