  ...
  try {
    ...
    if (paper.arxivId) {
      ...
      try {
        ...
        // 精确匹配 tar/gzip 类型
        const isTarGzip =
          contentType.includes('application/x-eprint') ||
        ...
        if (response.ok && isTarGzip) {
          ...
          if (buffer.byteLength < 512) {
            ...
            try {
              ...
              if (texFiles.length > 0) {
                console.log(
                  `[paper-download] OK: ${files.length} files, ${texFiles.length} .tex`
                );
          ...
          console.log(
            `[paper-download] e-print unavailable (status=${response.status}, type=${contentType})`
          );
        }
      } catch (e) {
      ...
      try {
        ...
        if (pdfResponse.ok && pdfContentType.includes('application/pdf')) {
          ...
          console.log(
            `[paper-download] PDF unavailable (status=${pdfResponse.status}, type=${pdfContentType})`
          );
        }
      } catch (e) {
