...
function addCronJob(agentId) {
    const data = readCronJobs();
    const jobId = `${agentId}-metabolism`;
    // Avoid duplicates
    if (data.jobs.some((j) => j.jobId === jobId))
        return;
    data.jobs.push({
...
function removeCronJob(agentId) {
    const data = readCronJobs();
    const jobId = `${agentId}-metabolism`;
...
function findPluginSkillsDir(entryDir) {
...
function listResearchProjects() {
    ...
    for (const agent of agents) {
        if (!agent.id.startsWith("research-"))
            continue;
...
function initProject(id, pluginSkillsDir) {
 ...
 */
export function registerResearchCli(api) {
    const entryDir = path.dirname(api.source);
    api.registerCli(({ program }) => {
        const cmd = program.command("research");
